...
import { updateMentorDetails } from "../../redux/slices/mentorSlice";
import {
  fetchCountries,
  fetchLanguages,
  fetchMentor,
...
export default function EditMentor() {
  ...
  const { loading, languages, countries, skills, mentor } = useSelector(
    (state) => state.data
  );
  ...
  useEffect(() => {
    dispatch(fetchMentor(user.email, navigate));
